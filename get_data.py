...
if not os.path.isdir(data_dir):
  print(f"{data_dir} will be created")
  os.makedirs(data_dir)

url = "https://catfact.ninja/fact"
headers = {"accept": "application/json", 
           "X-CSRF-TOKEN": "wLKZD6qfvKrN3wwx4LFol67YThBWc5cf4WbZJoAk"}

for i in range(n_facts):
  catfact = requests.get(url=url, headers=headers)
  timestamp = datetime.datetime.now().strftime(format="%Y-%m-%d:%H:%M:%S")
  
  catfact.raise_for_status()
  catfact_data = catfact.json()
  text = catfact_data["fact"]
  text = re.sub(r'[^\w\s]', '', text)
  words = text.split()
  filtered_words = [word for word in words if word.lower() not in stop_words]
  catfact_data["cleaned_fact"] = filtered_words
  filename = f"catfact_{i}.json"
  filepath = os.path.join(data_dir, filename)
  
  with open(filepath, "w") as f:
    json.dump(catfact_data, f)
